...
def compute_blob_vrange_at_phi(phi: float, delta_phi_deg: float,
    ...
    Args:
        ...
        torus_radius (astropy.units.Quantity): Central radius of the torus.
...
def compute_blob_vrange_over_phi(delta_phi_deg: float, Delta_r: u.Quantity,
    ...
    and radial width Delta_r, this function computes v_min, v_max, v_range,
    and the central point's radial velocity (v_center) as functions of the
    central blob angle phi spanning 0 to 2π, using rigid corotation.
    ...
    Returns:
        tuple: (phi_vals, v_min_vals, v_max_vals, v_range_vals, v_center_vals)
        where phi_vals are in radians and velocities in km/s.
    """
    ...
    period = 3.93 * u.hour
    Omega = 2 * np.pi / period
    v_center_val = (Omega * torus_radius).to(u.km / u.s).value
    v_center_vals = -v_center_val * np.sin(phi_vals)
    return phi_vals, v_min_vals, v_max_vals, v_range_vals, v_center_vals
...
def update_plots(torus_radius_val: float, delta_phi_deg: float,
    ...
    central blob angle, with a vertical line marking the current φ and an
    additional curve for the central point's radial velocity.
    ...
    polar_fig.add_trace(go.Scatter(
        x=[torus_radius_val * np.cos(phi_center_rad)],
        y=[torus_radius_val * np.sin(phi_center_rad)],
        mode='markers', name='Central Point',
    ...
    polar_fig.update_layout(
        ...
        xaxis=dict(constrain='domain', title="x (r_star)"),
        yaxis=dict(scaleanchor="x", scaleratio=1, title="y (r_star)"),
        title="Top-Down View of Annulus Segment"
    )

    # PANEL 2: Radial velocity vs. central blob angle.
    (phi_vals, v_min_vals, v_max_vals, v_range_vals,
     v_center_vals) = compute_blob_vrange_over_phi(
         delta_phi_deg, Delta_r_phys, torus_radius_phys,
    ...
    velocity_fig.add_trace(go.Scatter(
        x=phi_deg, y=v_center_vals, mode='lines',
        name='v_center (km/s)', line=dict(color='orange')
    ))
