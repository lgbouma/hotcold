...
and to a high incidence of flares 
...
6500\,\AA.  Strong winds contributed to
...
2024.  The HIRES spectra \added{(Panels b,c,e,f)} show emission in H$\alpha$ beyond the star's
...
distance of each clump from the stellar \added{rotation axis}: 2.07\,$R_\star$ and
...
\added{(see Figure~\ref{fig:fulllc} and Panels a and b of Figure~\ref{fig:spec})}.
...
visible in emission.  The origin of the other bright and dark
streaks passing across the line core \added{in Figure~\ref{fig:spec}e
}are similarly ambiguous.  \added{Figure~\ref{fig:spec}d shows} a
final exercise to quantify the behavior of the line core, where
$f_{\rm H\alpha\ core}$ denotes the summed \added{H$\alpha$} flux at
$|\Delta v / v_{\rm eq}|<1$.  This panel shows that changes in the
line core flux correlate with the broadband variability throughout
...
lower-velocity clump.  \added{Spectroscopic transits therefore do not
necessarily yield photometric transits.}
...
experienced by charged particles \added{along any given field line
}has a local minimum outside $R_{\rm c}$, which enables charged
material to accumulate in a torus.  Warps can occur when there is
misalignment between the spin and magnetic axes.  In general,
the\added{ regions} in these centrifugally-supported
...
scattering in a plasma transiting over a spotted background star might
...
would be to incorporate an opacity source that can also reproduce the
photometric dips.  It is likely that this effort will require
exploring what stellar magnetic field topologies reproduce the
observed light curve shapes \citep[e.g.][]{Bouma2024}. While previous
...
\citep{Brown2020}; such fields would yield accumulation surfaces quite
...
\citep{Jardine2002,Vidotto2011}. This might be alleviated through
future spectropolarimetric observations, or from radio observations
sensitive to the magnetic field structure around the star
\citep[see][]{Callingham2021,Kaur2024}.  Finally, the question of what
...
treatment of radiation propagating through such clumps remains to be
performed. The question of whether dust grains can nucleate in such
clumps, or whether they need to be externally sourced, deserves
careful attention.}
...
\begin{acknowledgements}
  ...
  %
	...
	2-minute cadence observations can be accessed \added{through MAST }via
