...
\revised{--}
\accepted{--}
\shorttitle{M Dwarf Plasma Torus} 
...
time-varying Balmer emission at twice to four times the star's
equatorial velocity, providing direct evidence for cool
($\lesssim$10$^4$\,K) plasma clumps trapped in corotation around a CPV.
These data support the idea that young, rapidly-rotating M dwarfs can
sustain warped plasma tori, similar to massive magnetic stars.  The
origin of the optically thick material in these tori, and the issue
of whether this material contains a significant amount of dust,
remain unclear.
\end{abstract}
...
\citep[see][]{Redfield2024,TRAPPIST1JWSTCommunityInitiative2024}.  The
question of how an M dwarf's evolution influences its
planets---especially the retention of their atmospheres---has in turn
become a major theme in exoplanet and stellar astrophysics.  Previous
work has established that most M dwarfs host close-in planets
\citep{Dressing2015} that on average are subject to long circumstellar
disk lifetimes \citep{Ribas2015}, to high doses of UV radiation
\citep{France2016}, and to a high incidence of flares and coronal mass
ejections \citep{Feinstein2020}.  However, despite extensive work in
these areas, the plasma and magnetospheric environments that bathe
young, close-in exoplanets remain challenging to quantify.
...
$\approx$0.3\% by $\approx$150\,Myr \citep{Rebull2020}.  CPVs are a
...
the material exists, then the geometric correction from the transit
...
corotating clumps of cool plasma around a CPV, TIC\,141146667.  The
observations are presented in Section~\ref{sec:obs}, with the main new
evidence in Figure~\ref{fig:spec}.  Section~\ref{sec:results} expounds
on the results; Section~\ref{sec:disc} discusses their interpretation
...
star for spectroscopy because its brightness and rotation rate enabled
an efficient search for variability in its line profiles.
As apparently single pre-main sequence M dwarf, its properties are
typical for the general CPV population (see Table~\ref{tab:params} and
...
\citep{Howard2010}.  Figure~\ref{fig:spec} shows the result in the
vicinity of H$\alpha$ without any additional processing.

To date, TESS has observed TIC\,141146667 ($T$=13.3) for a total of six
non-contiguous months spanning 2019-2024.  Two-minute cadence observations were
acquired during Sectors 41, 48 (TESS DDT039, PI: Kunimoto), and 75
(TESS Program G06030, PI: Bouma); the remaining 30-minute data during
Sectors 14, 15, and 21 smears sharp features over the $<$4\,hour
period (see \citealt{Gunther2022}).  The concatenated two-minute data
are shown in the movie associated with Figure~\ref{fig:lc}.  The movie shows that,
...
TIC\,141146667 and is photometrically stable in the TESS target pixel
cutouts; crowding is not a concern.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/sf1.pdf}
  \vspace{-0.2cm}
  \caption{{\bf Evolution of TIC\,141146667 during the Keck/HIRES
  observation (green bar).}  {\bf a,} TESS simple aperture photometry.
  Data gaps caused by light from the Earth (BTJD 3356-3358.5) and Moon
...
the Earth passed within 25$^\circ$ of the
TESS camera's boresight from BTJD 3356.0~-~3358.5, which caused a data
gap that ended twelve hours after the Keck/HIRES observation epoch
(green bar).  From
...
out times from 3359.40~-~3360.13; the remainder of the data during the
lunar approach were usable.  Small data gaps from BTJD
...
that a large, complex flux decrement spanned roughly half of each
...
readily explained as a plasma being dragged along
...
visible in emission.  Although the origin of the other bright and dark
...
clumps exist around at least one such star.  While starspots likely do
contribute smooth signals to the photometric variability, a
``starspot-only'' scenario for the CPVs \citep{Koen2021} is
incompatible with our observations.  Scenarios in which the
circumstellar material is made only of dust are similarly ruled out.
While starspots and dust may both be present, the observed H$\alpha$
emission requires plasma with a significant population of hydrogen
atoms in the $n$=3 excited state.  Most of this H$\alpha$ emission
originates in ``clumps'' with size similar to that of the star; radial
``spokes'' or azimuthally-extended ``arcs'' for the emitting material are
ruled out by the observed H$\alpha$ velocity dispersion (see
\S~\ref{subsec:halpha}).  
The observed H$\alpha$ luminosity suggests characteristic densities
and masses for the gaseous component of these clumps of $n_{\rm H}
\sim 10^{11}$\,cm$^{-3}$ and $M_{\rm gas} \sim 10^{17}$\,g (see
\S~\ref{subsec:gas}).  Dust, if present, is independently constrained
to have a total mass $10^{15}\,{\rm g} < M_{\rm dust} < 10^{17}\,{\rm
g}$ (see \S~\ref{subsec:dust}).  
The spectra show minimal evidence for significant populations in
higher-order hydrogen states (see Appendix~\ref{subsec:specvar}).
