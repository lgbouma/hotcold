...
\begin{figure}[!tp]
  ...
  Underplotted sinusoids are to guide the eye.
...
spectral feature around the priors discussed above, and sometimes
these features are not statistically significant.  During such
spectral epochs, e.g.~the ``outer'' clump at $\phi$=1.07, the model is
fitting noise, not signal.  
At times the model underfits.  For instance, the sudden blue rise near
$\phi$=0.2 is not well-matched by the functional form of a gaussian;
the assumed functional form is one of convenience.
A final point of caution is that the model fits the $i=\{ 0, 1 \}$,
and $i=2$ components independently.  At $\phi=0.0$ and $1.0$,
Figure~\ref{fig:halphamodel} suggests that this means the emission might
hypothetically be from either the inner or outer components.  A
physical interpretation however is that this is the phase during which
the inner component is in transit, and the outer is in secondary
occultation and blocked by the star.  At $\phi=0.0$, the double-peaked
emission profile also matches that seen shortly afterward when the
inner clump is viewed off-disk.  A physical interpretation of the
model should therefore discard the results at this particular epoch
for the outer clump.

Taking these caveats into account, the results from Extended Data
Table~\ref{tab:halphamodelparams} can be used to evaluate various
models for the orbits.

, and in particular to assess their periods and
semi-amplitudes.
