...
While the H$\alpha$ emission seems to mostly originate from clumps,
the complex flux dip in Figure~\ref{fig:spec} spans roughly half of
each cycle; in CPVs more broadly, individual dips can last 5-50\% of
each cycle \citep{Bouma2024}.  This implies that
...
in TIC\,141146667 and CPVs more broadly is that {\it i)}
Figure~\ref{fig:spec} shows two emitting plasma clumps separated
180$^\circ$ in phase, with double-peaked H$\alpha$ morphology at
quadrature; {\it ii)} ``W'' shaped photometric eclipses are
predictions of such a model; {\it iii)} CPV dips last 5-50\% of each
cycle, suggesting an azimuthally-extended distribution of circumstellar
material.  \added{Two key challenges however are that the warped torus
model predicts two W-shaped eclipses per cycle, and it also predicts
that the antipodal clumps should be equidistant from the star.
Plausible resolutions could invoke different opacities for each clump, 
and non-dipolar stellar magnetic fields.
Generally, the evidence seems to support a warped torus as a
reasonable first approximation, which we hope will be refined by
future efforts.}
...
atmosphere models \citep{Allard2012}, assuming the \citet{Asplund2009}
solar abundances and the \citet{Barber2006} water line lists.  This
...
standards tabulated by \citet{Bochanski2007}.   This result agrees with
...
accumulate (e.g.~\citealt{Townsend2005,Daley-Yates2024}).  Our final
