...
\cite{Ribas2015}, to high doses of UV radiation \cite{France2013},
...
chromosphere; circumstellar material might also modulate the
line profile.
...
amplitude greater than that observed from the circumstellar components,
and it crosses the stellar velocity surface too slowly to be
circumstellar.
...
%
$r_0$\dotfill                           & Inner clump (0) orbital radius ($R_\star$)\hspace{9pt}\dotfill         &  2.07$\pm$0.04                 & 4 \\
$r_1$\dotfill                           & Inner clump (1) orbital radius ($R_\star$)\hspace{9pt}\dotfill         &  2.88$\pm$0.10                 & 4 \\
$r_2$\dotfill                           & Outer clump orbital radius ($R_\star$)\hspace{9pt}\dotfill         &  4.16$\pm$0.56                 & 4 \\
%
...
external sources include an undetected old disk, comets, or a
...
variables, a rare subset of B stars with radiatively-driven winds which
...
The material's composition -- either pure plasma or dusty
...
dwarf wind, similar to that formed around more evolved stars
\cite{Marigo2008}.
...
rotating ones with centrifugal magnetospheres -- host 
circumstellar environments similar to the CPVs.
...
region, and can also clarify the degree to which the spectroscopic
period agrees with the photometric period.
...
We opted to label the inner component's two gaussians $i=\{ 0, 1 \}$,
...
the model is fitting noise, not signal.  At other times, the model
...
orbits; considering the Bayesian information criteria, we found no
...
However, at quadrature, the observed velocity width is sensitive to the
radial extent of the circumstellar material.  At mid-transit, the
observed velocity width is sensitive to the azimuthal extent.  The arc
configuration would minimize the observed velocity width $\sigma$ at
quadrature, and maximize it during transit, with $\gtrsim$100\,\kms\ 
variations in between.  Since this is not observed the arc geometry,
and by a similar argument the spoke geometry, can be discarded.  
...
constraint does come from the TESS transit depth scaling with
the projected area $\ell z$.  More specifically, one can evaluate an
``effective area'' blocked by a two-dimensional gaussian blob passing over a star by
integrating the local gaussian weight over the stellar surface.  For
instance, if $\ell$$\approx$$z$$\approx$0.24\,$R_\star$, then
$\iint \exp \left(
