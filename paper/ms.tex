...
of this optically thick material and its microphysical opacity remain
unclear.
\end{abstract}
...
The H$\alpha$ line core is more complex.  At $|\Delta v /
...
\begin{figure}[!tp]
  ...
  core.  
  Section~\ref{subsec:halpha} discusses the use of this model.
  }
...
exist around TIC~141146667, it would be valuable to quantify the exact
orbital periods, velocities, and velocity dispersions of these clumps.
These quantities can clarify the precision with which the
spectroscopic period agrees with the photometric period, and also
constrain the physical dimensions of the emitting region.
...
Figure~\ref{fig:halphamodel}; details in its implementation and
interpretation follow.

To the implement the model, we assumed that the ``inner'' ($K_{\rm
inner}$$\approx$2.5\,$v_{\rm eq}$) clump would be well-fit by a sum of
two gaussians because it visually appears double-peaked in the raw
data from $\phi$=0.15-0.35 and $\phi$=0.65-0.85
(Figure~\ref{fig:spec}b).  We assumed the ``outer'' ($K_{\rm
...
Denote the two gaussians for the inner component $i=\{ 0, 1 \}$,
and the single outer component with $i=2$.  Given the complexity of the
...
for the outer component.  This prior on the means mitigates the
multimodality of the likelihood by requiring the mean velocity of each
...
Caution is required in interpreting this model's results.  At times
the model overfits, because sometimes the most significant spectral
feature around any component's prior is not statistically significant.
During such spectral epochs, e.g.~the ``outer'' clump at $\phi$=1.07,
the model fits noise, not signal.  At other times, the model
underfits.  For instance, the sudden blue rise near $\phi$=0.2 is not
well-described by a gaussian; the assumed functional form is one of
...
Physically however, at this time the inner component
is in transit, and the outer component is in secondary eclipse and
would likely be blocked by the star.  At $\phi=0.0$, the double-peaked
...
model should therefore discard the outer clump results at this
particular epoch.
...
\begin{figure}[!t]
  ...
  Each marker represents the measured best-fit mean at a given epoch.
  Solid circles were used in the fits; transparent X markers
  were excluded due to concerns about their epoch-by-epoch
  interpretation (see text).  The purpose of this exercise was to
  quantify the orbital periods and semi-amplitudes of the
  circumstellar clumps.  The inner double-peaked clump is shown in
  orange and blue; the outer clump is shown in green.  Five draws from
  each posterior probability distribution are plotted.}
  \label{fig:orbits}
\end{figure}

Taking these caveats into account, we used the results from Extended
Data Table~\ref{tab:halphamodelparams} to evaluate various models for
the orbits.  Our main goal in this exercise was to quantify the
orbital periods and velocity semi-amplitudes of the clumps.
Figure~\ref{fig:orbits} shows representative results for the
circular orbit case; we found no reason to extend the exercise to
eccentric orbits.
Based on visual inspection of the line profile fits, we manually excluded 
epochs where we do not believe our gaussian profile fitting provided a
reliable representation of the circumstellar emission.  We then performed
the orbit fits using NumPyro to sample over the Gaussian likelihood with the
NUTS algorithm \cite{Phan2019}.  When performing the fits, we adopted
the uncertainties on each estimated mean radial velocity value, and
added a jitter term in quadrature to inflate these nominal measurement
uncertainties.  This procedure yielded period and semi-amplitude
values for circular orbits of
$P_0$=$3.92\pm0.03$\,hr, $K_0/v_{\rm eq}$=2.07$\pm$0.04;
$P_1$=$3.92\pm0.06$\,hr, $K_1/v_{\rm eq}$=2.88$\pm$0.10;
and $P_2$=$3.35\pm0.35$\,hr, $K_1/v_{\rm eq}$=4.16$\pm$0.56.
The periods for the inner double-peaked clump are consistent
with the photometric 3.930$\pm$0.001\,hr period to a precision of
about two minutes.
The ``period'' for the outer clump is more ambiguous because the
H$\alpha$ data only support the idea of a periodic orbit of material
well-fit by gaussian emission from $\phi$$\approx$0.5-1.0.
From $\phi$=0-0.2, there is no detectable emission,
and from $\phi$=0.2-0.4, the suddenly-detectable emission spans
1-4\,$v_{\rm eq}$ without taking a clear gaussian shape.
While the outer-most edge of the $\phi$=0.2-0.4 emission provides a
closer match to the expectation of a circular orbit, 
the idea of invoking a particular functional form for this component 
seems fine-tuned.
We instead emphasize that although this emission is present, 
its variability in time is poorly explained by the idea of a
stable, long-lived clump of material.
Although additional observations would be needed to conclusively determine whether or not 
this component of the system
is long-lived, the lack of any photometric variability in the TESS
data during its expected transits might suggest that it is not.


{\it Constraints on physical dimensions of emitting region}---
%TODO FIXME TODO FIXME


IMPLICATIONS:
The secondary eclipse of the `inner clump' at $\phi$$\approx$0.5 is not
associated with a broadband flux dip.   The clump does not therefore
contribute significant broadband emissivity across the entire TESS
band; only in H$\alpha$.
...
%\begin{table}
  ...
  subset of the spectral epochs provided statistically significant
