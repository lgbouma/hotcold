...
Given that a full radiative transfer simulation was beyond our scope,
...
This latter assumption constrains the mean velocity of each component
to be near the value suggested by visual inspection, but provides
sufficient flexibility for the model to deviate from the assumption of
a sinusoid if the data dictate such behavior.
We fitted each component to the data {\it independently} using scipy's
non-linear least squares \texttt{curve\_fit} method
\cite{Virtanen2020}, and scaled the resulting parameter covariance
matrix by a constant factor to match the sample variance of the
residuals.
The choice to fit the $i=\{ 0, 1 \}$, and $i=2$ components
independently generally did not affect the results compared to if we
have fit all three simultaneously, except for at $\phi=1.00$ (see
...
%\begin{table}
  ...
  \hline
   & Mean1 & Mean2 & Mean3 & Sigma1 & Sigma2 & Sigma3 & Amplitude1 & Amplitude2 & Amplitude3 \\
  BTJD &  &  &  &  &  &  &  &  &  \\
  \hline
  ...
  \hline
  \end{tabular}
  \caption{Multi-gaussian model fit results to H$\alpha$ line profiles.}
