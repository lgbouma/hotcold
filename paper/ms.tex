...
Accounting for these caveats, we used the results from Extended
Data Table~\ref{tab:halphamodelparams} to evaluate various models for
the orbits.  Our main goal was to quantify the orbital periods and
velocity semi-amplitudes of the clumps.
Figure~\ref{fig:orbits} shows representative results for the case of a circular
orbit; eccentric orbits yielded similar results.  Based on visual
inspection of Extended Data Figure~\ref{fig:halphamodel}, we manually excluded
epochs when our gaussian profile fitting failed to either detect or
...
The periods for the inner double-peaked clump are consistent with the
...
although this emission is present, its variability in time is poorly
explained by the idea of a stable, long-lived clump of material.
Although additional observations would be needed to conclusively
determine whether or not this component of the system is long-lived,
the lack of any photometric variability in the TESS data during its
expected transits suggests that it is not.


{\it The physical dimensions of emitting region}---The measured
velocity widths of the emitting circumstellar material contain
information about the size of the emitting region via the condition
for rigid corotation.
Consider a clump in cylindrical coordinates with arbitrary radial
extent $r$, azimuthal extent $\ell$, and height $z$.
At quadrature, the observed velocity width is sensitive to the radial
extent of this material because it is being viewed face-on.
During transit, the observed velocity width is sensitive to the
azimuthal extent.
Although the vertical height is less constrained for an edge-on system, 
a rough estimate can follow from assuming that the depth of
the broadband TESS absorption $\delta$ should scale with the product
$\ell z$.
...
radial extent $r_i$=$2\sigma_i / \Omega$$=$0.48\,$R_\star$, and
that 95\% of the emission comes from within 0.96\,$R_\star$.
These values have relative uncertainties of $\approx$5\%, based on the
uncertainties in the measured velocity
dispersions.  The two inner clumps are centered at distances of
2.07\,$R_\star$ and 2.88\,$R_\star$.  There is therefore 
physical overlap in their spatial distributions.  These
two clumps could in fact be a single clump with an optically thick H$\alpha$
line core.
Regardless of this nuance, the implication is that the combined 
extent in the radial dimension of these two inner emitting clumps is
roughly equal to the star's diameter.
At mid-transit, these inner clumps seem to have a similar velocity
dispersion, although with greater uncertainty due to the differences
between the $\phi$=0 and $\phi$=1 transits.  This nonetheless suggests
a 1$\sigma$ emission contour in the azimuthal dimension of
$\ell$$\approx$0.5\,$R_\star$.

%TODO FIXME ADD DISCUSSION OF VERTICAL DIMENSION...
