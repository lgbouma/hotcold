...
{\it TESS:} TESS observed TIC~141146667 ($T$=13.3) in Sectors 14, 15,
21, 41, 48, and 75.  Two-minute data were acquired during Sectors
41, 48 (TESS DDT039, PI: Kunimoto), and 75 (TESS Program G06030, PI:
Bouma).  The data from Sectors 14, 15, and 21 had a 30-minute
cadence, which significantly phase-smears sharp features over the
$<$4\,hour period (see \cite{Gunther2022}).  The nearest known star,
TIC~141146666 ($T$=14.5), is 25$''$ from TIC~141146667 and based on
the pixel-level TESS data is photometrically stable.

Figure~\ref{fig:fulllc} shows the simple aperture photometry from
Sector~75, which were acquired using the camera that points closest
to the ecliptic.  The largest gap in coverage is from BTJD
3356.0~-~3358.5, and includes the Keck/HIRES observation epoch
(green bar).   There are no flux measurements reported during this
interval because the Earth was within 25$\^circ$ the camera's
boresight, yielding high levels of scattered light.  From BTJD
3359.4~-~3362.0, the Moon also passed within 25$^\circ$ of the
camera's boresight.  Based on the observed level of scattered light
in the local aperture of TIC~141146667, we therefore manually masked
out times from 3359.4~-~3360.13, and judged the remainder of the
data to be usable.  The small data gaps from BTJD 3353.55~-3353.77
and from BTJD 3360.12~-~3360.33 were caused by data downlinks at the
spacecraft's perigee and apogee, respectively.

{\it Keck/HIRES:}
We observed TIC~141146667 ($V$=16.2) with HIRES for five hours over a
...
in the spatial dimension, yielding $\approx$1,000 counts (S/N=33)
...
Figure~\ref{fig:spec} shows a small segment of the result without
any additional processing.


\subsection{Spectroscopic Variability}

The full wavelength coverage spans 3650-7960\,\AA, and variable
...
for co-moving ($\Delta v__{\rm T}$$<$5\,\kms) stars that share its
isochronal youth.
