...
demonstrating the existence of corotating clumps of circumstellar
...
condensations can sustain an optically thick component; the 
origin of the optically thick material and its microphysical opacity
remain unclear.
\end{abstract}
...
Investment with JWST has been proceeding accordingly
...
their atmospheres--- has concurrently become a major theme in
...
on average subject to long circumstellar disk lifetimes \cite{Ribas2015},
to large doses of UV radiation \cite{France2013}, and to a high
incidence of flares and coronal mass ejections \cite{Gunther2020}.
However, despite excellent work in these areas, the properties of the
plasma and magnetospheric environments that bathe young, close-in
exoplanets have remained challenging to quantify.
...
These CPVs are
phenomenologically identified based on their structured, periodic
...
behavior, an  observed fraction which decreases to $\approx$0.3\%
...
\cite{Koen2021}.  
The main argument against a starspot-only explanation invokes the
timescales and amplitudes of the sharpest photometric features.
However, no independent evidence has yet been acquired for the
presence of any circumstellar material.  Geometrically, transiting
clumps would imply an occurrence a few to at most ten times the
observed rate.  The clumps could therefore exist around 10-30\% of M
dwarfs during their early lives.
...
cool ($10^4$ K) plasma. In particular, time-series spectra of
stars with a wide range of masses have shown periodic high-velocity
...
Such clumps are thought to be forced into corotation by the magnetic field, and the
...
In this study, we present the first detection of corotating clumps of
...
selected the star for spectroscopy because its brightness and 
rotation rate enable efficiently mapping out any line profile variability.  We
...
Keck I 10m telescope.  TESS observed the star over a four week interval
...
epoch of observation; the general photometric morphology is similar before and
after the data gap.
...
\begin{figure}[!tp]
  ...
  {\bf Panel b:} Keck/HIRES H$\alpha$ spectra acquired on UT 17
...
to be generated in the star's chromosphere
...
Based on the observed H$\alpha$ luminosity, the characteristic
densities and masses of the gaseous component of these clumps are
$n_{\rm H} \sim 10^{11}$-$10^{12}$\,cm$^{-3}$ and $M_{\rm gas} \sim 10^{17}$\,g
(see Section~\ref{subsec:gas}).
Any dust, if present, is independently constrained to have a total
...
eclipsing binaries, and ``S-wave'' H$\alpha$ spectra similar to those
...
(e.g. \cite{Callingham2021}) would provide a measurement of the field
...
We caution however that the following estimates are coarse: detailed
...
Assuming a uniform density clump, this yields a total gas mass $M_{\rm gas}\approx
8\times10^{16}$\,g.

Finally, a constraint on the magnetic field strength at the site of the clump
...
if it were significantly beyond 1,000-10,000\,K,
...
The material's exact composition -- either pure plasma, or else a
dusty plasma -- is not known.  In particular, the idea of dust being
present seems plausible given observations of chromatic transits in
analogous objects \cite{Tanimoto2020,Gunther2022,Koen2023}.
This scenario is highly constrained.  An upper limit on the amount of
...
typically have gas masses of order $10^{14}$\,g-$10^{16}$\,g
...
Broadly, our calculations suggest that if dust is responsible for the
...
Section~\ref{subsec:gas}, this range spans mass ratio ranges $M_{\rm
gas}/M_{\rm dust}$ of 100 to 1.  More careful measurements of this
ratio---in particular enabled by inferring the dust mass through high
precision infrared spectrophotometry---could provide a pathway for
distinguishing the scenario of a stellar outflow from an 
accumulation of extrinsically-sourced material.
...
\item[Acknowledgments]
  LGB is grateful to M.~Jardine for insightful conversations,
  A.~Weinberger for clarification of infrared dust spectroscopy,
  B.~Tofflemire for a clarifying line profile analysis, and to both
  A.~Howard and H.~Isaacson for their assistance reducing the HIRES
  spectra.
  ...
  thousands of people, and we thank the many TESS team members for
  their efforts to make the mission a continued success.
