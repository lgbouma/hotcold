...
our HIRES spectra using a pipeline that we developed specifically for
...
epochs on 17~February~2024 (UT) was $\gamma$=0.6$\pm$1.5\,\kms.  The
...
\begin{figure}[!t]
  ...
  \caption{
    ...
    \cite{Allard2012}.  This plot was made using \texttt{astroARIADNE}
    \cite{Vines2022}.  The photometry extends from the Gaia DR3 blue
    passband to WISE W3;  the redder (22 $\mu$m) W4 passband did not yield
    ...
    model sets an upper limit on emission from any circumstellar dust.
    }
  \label{fig:sed}
\end{figure}
...
absorption and scattering by dust, rather than any radiative processes
...
excess at 10\,$\mu$m (W3).  A simple approximation is to assume $T_{\rm
dust}$=1500\,K, typical for dust near the star (see \cite{Zhan2019} for
discussion regarding dust sublimation).  We then treat emission from the
dust and the star as Planck functions, requiring $F_{\lambda,\star} > f
\cdot F_{\lambda,\mathrm{dust}}$, for $f$ the scaling factor set by the
WISE photometric uncertainty.  For this condition to hold, we find that
the bolometric flux ratio $F_{\rm dust}/F_{\star}$$<$6$\cdot$10$^{-3}$.
Scaling the associated luminosities ($L = \sigma_{\rm SB} T_{\rm eff}^4
/ (4\pi R^2)$) implies an emitting area for the dust $<$10\% of the
star's surface area.  Converting this constraint to a dust mass requires
an assumption regarding the grain properties; taking a grain density
$\rho$=3\,g\,cm$^{-3}$ typical for silicate grains, and a fixed size
$a$=1\,$\mu$m \cite{Tanimoto2020,Gunther2022,Koen2023} yields an upper
limit of $M_{\rm dust}\lesssim$10$^{17}$\,g.

By way of comparison, the lower limit follows 
%FIXME TODO finish!!
...
which implies a 
Since the effective temperatures 
...
, reporting $W4$=8.4 with no uncertainty.  
The WISE images, available through the IRSA archive, concur.



%WISE_RSR_W3 9.8380 0.0450
%W4 = 8.392 +/- nan
